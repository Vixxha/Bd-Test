...
import { CrmPowerBiView } from '@/views/CrmPowerBiView';

import { 
  LayoutDashboard, ShoppingBag, Package, Tag, Truck, BarChart2, 
  ShoppingCart, History, Users, Settings, Moon, Sun, LogOut, Globe, Database
} from 'lucide-react';
import './App.css';

type Tab = 'dashboard' | 'web-orders' | 'inventory' | 'categories' | 'suppliers' | 'admin' | 'pos' | 'sales-history' | 'vendors' | 'settings' | 'web-content' | 'crm-powerbi';
...
export default function Home() {
  ...
  return (
    <div className={`app-layout-dashboard ${darkMode ? 'dark-mode' : ''}`}>
      {/* Barra Lateral Izquierda (Chile Frutos Secos Sidebar) */}
      <aside className="rofex-sidebar">
        <div className="sidebar-brand-block">
          <img 
            src="/logo.png" 
            alt="Chile Frutos Secos Logo" 
            style={{ 
              width: '36px', 
              height: '36px', 
              borderRadius: '8px', 
          ...
          />
          <strong style={{ marginLeft: '12px', fontSize: '14px' }}>Frutos Secos</strong>
        </div>
        ...
        <nav className="sidebar-nav-menu">
          ...
          <button 
            type="button"
            onClick={() => setActiveTab('crm-powerbi')} 
            className={`sidebar-nav-btn ${activeTab === 'crm-powerbi' ? 'active' : ''}`}
          >
            <Database size={18} />
            <span>CRM & Power BI</span>
          </button>
      ...
      <div className="main-content-wrapper">
        ...
        <header className="main-topbar">
          <div className="topbar-right-profile">
            ...
            <div className="profile-text-info">
              ...
              <span className="profile-user-role">CFS POS</span>
            </div>
            <div className="profile-avatar-circle" style={{ backgroundColor: '#2D6A4F' }}>C</div>
          </div>
        </header>
        ...
        <main className="view-viewport-content">
          ...
          {activeTab === 'crm-powerbi' && <CrmPowerBiView />}
        </main>
      </div>
    </div>
  );
